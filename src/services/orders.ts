import { deliveries, orders, type Order, type OrderItem } from '../data.js';
import { deliveries as deliveriesList } from '../data.js';
...
export function buildRecurringReminders(items: Order[]) {
  return items
    ...
    .map((o) => ({
      orderId: o.id, patientName: o.patientName, nextBillingDate: o.recurring?.nextBillingDate,
...
export function getOrdersPaginated(page: number, pageSize: number) {
  ...
  return { items: orders.slice(start, end), page, pageSize, total: orders.length, totalPages: Math.max(1, Math.ceil(orders.length / pageSize)) };
}
...
export function getDeliveriesPaginated(page: number, pageSize: number, status?: string, q?: string) {
  ...
  if (q) {
    ...
    filtered = filtered.filter((d) => d.orderId.toLowerCase().includes(lower) || d.patientName.toLowerCase().includes(lower));
  }
  const start = (page - 1) * pageSize;
  const end = start + pageSize;
  return { items: filtered.slice(start, end), page, pageSize, total: filtered.length, totalPages: Math.max(1, Math.ceil(filtered.length / pageSize)) };
}
