...
export function createApp() {
  ...
  app.post('/api/orders/:orderId/recurrence-confirmation', (req, res) => {
    ...
    order.recurring.confirmedAt = new Date().toISOString();
    order.recurring.confirmedBy = parsed.data.confirmedBy;

    return res.json({ message: 'Recorrência confirmada com o cliente.', orderId: order.id, recurring: order.recurring });
  });
  ...
  app.get('/api/dashboard/:role', (req, res) => {
    ...
    const reminders = orders
      .filter((o) => {
        const recurring = o.recurring;
        if (!recurring || recurring.confirmedAt) return false;
      ...
      .map((o) => ({
        ...
        message: 'Aviso: faltam até 3 dias para o faturamento. Confirme a recorrência com o cliente.'
      }));
