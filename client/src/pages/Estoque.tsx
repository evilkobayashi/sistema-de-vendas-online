...
export default function Estoque() {
  ...
  const load = async () => {
    const [s, m] = await Promise.all([
      api.get('/inventory/summary?page=1&pageSize=50') as Promise<InventorySummary>,
      api.get('/medicines') as Promise<MedicinesResponse>,
    ]);
    setSummary(s);
  ...
  const handleAddMedicine = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload = Object.fromEntries(fd.entries()) as Record<string, string | boolean>;
